...
const validate = {
    ...
    ConfirmPassword: {type: "string", required: true, message: '登录密码不能为空'},
...
class SysUserLogin extends PureComponent {
    ...
    handleOperator = () => {
        ...
        this.core.validate((err) => {
            if (!err) {
                this.setState({displayEnterprise: 'block', text: '注册'})
    ...
    return (
        <Card title="注册表单">
            <Form core={this.core} layout={{label: 7}}>
                ...
                <br/>
                <FormItem><Button type="primary" onClick={this.handleOperator}>{this.state.text}</Button></FormItem>
            </Form>
        </Card>
    );
}
