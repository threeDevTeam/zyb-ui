...
class AlikeSickOfEnterprise extends PureComponent {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            ...
            request('/zyb/alikeSickOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res && res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/alikeSickOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res&&res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/alikeSickOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res&&res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/alikeSickOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
