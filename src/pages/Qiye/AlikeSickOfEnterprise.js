...
//
import AlikeSickOfEnterpriseDemoForm from '../QiyeDemoFrom/AlikeSickOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class AlikeSickOfEnterprise extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zybadmin/alikeSickOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zybadmin/alikeSickOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/alikeSickOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/alikeSickOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zybadmin/alikeSickOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
                ...
                <Table onRow={record => {
                    ...
                    <Table.Column title="检查机构" dataIndex="org"/>
                    <Table.Column title="检查日期" dataIndex="checkDate"/>
                    <Table.Column title="检查年份" dataIndex="checkYear"/>
                    <Table.Column title="检查月份" dataIndex="checkMonth"/>
                    <Table.Column title="发病工龄" dataIndex="sickYear"/>
                    <Table.Column title="是否进行了疑似职业病病人报告" dataIndex="isReport"/>
...
export default AlikeSickOfEnterprise