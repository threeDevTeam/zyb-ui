...
//
import AccidentPersonOfEnterpriseDemoForm from '../QiyeDemoFrom/AccidentPersonOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class AccidentPersonOfEnterprise extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <AccidentPersonOfEnterpriseDemoForm option={{type}}/>,
                onOk: (values, hide) => {
                    if (values.dieDateStr) {
                        values.dieDateStr = values.dieDateStr.format('YYYY-MM-DD')||  values.dieDateStr._i
                    }
                    request.post('/zyb/accidentPersonOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zyb/accidentPersonOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res&&res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/accidentPersonOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res&&res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/accidentPersonOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res&&res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/accidentPersonOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
                ...
                <Table onRow={record => {
                    ...
                    <Table.Column title="是否死亡" dataIndex="isDie"/>
...
export default AccidentPersonOfEnterprise