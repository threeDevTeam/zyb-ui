...
//
import PersonProtectOfEnterpriseDemoForm from '../QiyeDemoFrom/PersonProtectOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class PersonProtectOfEnterprise extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <PersonProtectOfEnterpriseDemoForm option={{type}}/>,
                onOk: (values, hide) => {
                    request.post('/zyb/personProtectOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zyb/personProtectOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <PersonProtectOfEnterpriseDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            request.post('/zyb/personProtectOfEnterprise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/personProtectOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                locale: 'zh',
                style: {width: '400px'},
                content: `确定要删除个人防护用品名称=${this.state.record.name}的数据吗?`,
                onOk: (values, hide) => {
                    request('/zyb/personProtectOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/personProtectOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
...
export default PersonProtectOfEnterprise