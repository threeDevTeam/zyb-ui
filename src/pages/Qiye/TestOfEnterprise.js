...
//
import TestOfEnterpriseDemoForm from '../QiyeDemoFrom/TestOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class TestOfEnterprise extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <TestOfEnterpriseDemoForm option={{type}}/>,
                onOk: (values, hide) => {
                    request.post('/zyb/testOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zyb/testOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <TestOfEnterpriseDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            request.post('/zyb/testOfEnterprise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/testOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/testOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/testOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
                ...
                <Table onRow={record => {
                    ...
                    <Table.Column title="体检类别" dataIndex="type"/>
                    <Table.Column title="体检结果" dataIndex="result"/>
                    <Table.Column title="处理意见" dataIndex="note"/>
                    <Table.Column title="落实情况" dataIndex="implement"/>
                </Table>
                <Pagination/>
            </List>
        )
...
export default TestOfEnterprise