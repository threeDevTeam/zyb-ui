...
//
import PersonOfEnterpriseDemoForm from '../QiyeDemoFrom/PersonOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class PersonOfEnterprise extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <PersonOfEnterpriseDemoForm option={{type}}/>,
                onOk: (values, hide) => {

                    request.post('/zybadmin/personOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zybadmin/personOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <PersonOfEnterpriseDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            request.post('/zybadmin/personOfEnterprise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/personOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/personOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zybadmin/personOfEnterprise/list'onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
...
export default PersonOfEnterprise