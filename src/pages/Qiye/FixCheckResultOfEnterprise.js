...
//
import FixCheckResultOfEnterpriseDemoForm from '../QiyeDemoFrom/FixCheckResultOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class FixCheckResultOfEnterprise extends PureComponent {
    state = {}
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <FixCheckResultOfEnterpriseDemoForm option={{type}}/>,
                onOk: (values, hide) => {
                    request.post('/zybadmin/fixCheckResultOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zybadmin/fixCheckResultOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <FixCheckResultOfEnterpriseDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            request.post('/zybadmin/fixCheckResultOfEnterprise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/fixCheckResultOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zybadmin/fixCheckResultOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
...
export default FixCheckResultOfEnterprise