...
class AccidentSumOfEnterprise extends PureComponent {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zyb/accidentSumOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
    ...
    render() {
        return (
            <List url='/zyb/accidentSumOfEnterprise/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
