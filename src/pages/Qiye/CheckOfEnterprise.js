...
//
import CheckOfEnterpriseDemoForm from '../QiyeDemoFrom/CheckOfEnterpriseDemoForm'
...
// @connect(({demo}) => ({demo}))
class CheckOfEnterprise extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    request.post('/zyb/checkOfEnterprise/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zyb/checkOfEnterprise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <CheckOfEnterpriseDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            if (values.checkDateStr) {
                                values.checkDateStr = values.checkDateStr.format('YYYY-MM-DD')
                            }
                            request.post('/zyb/checkOfEnterprise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/checkOfEnterprise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                locale: 'zh',
                style: {width: '400px'},
                content: `确定要删除检查部门=${this.state.record.org}的数据吗?`,
                onOk: (values, hide) => {
                    request('/zyb/checkOfEnterprise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/checkOfEnterprise/list'onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
                    <Filter.Item label="检查部门" name="org"><Input/></Filter.Item>
                ...
                <Table onRow={record => {
                    ...
                    <Table.Column title="是否接受过相关部门检查" dataIndex="isAccept"/>
                    <Table.Column title="检查时间" dataIndex="checkDate"/>
                    <Table.Column title="检查部门" dataIndex="org"/>
...
export default CheckOfEnterprise