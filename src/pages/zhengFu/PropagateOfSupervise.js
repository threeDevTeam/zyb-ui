...
//
import PropagateOfSuperviseDemoForm from '../ZhengfuDemoFrom/PropagateOfSuperviseDemoForm'
...
class PropagateOfSupervise extends PureComponent {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zyb/propagateOfSupervise/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            request('/zyb/propagateOfSupervise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        onOk: (values, hide) => {
                            request.post('/zyb/propagateOfSupervise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/propagateOfSupervise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/propagateOfSupervise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
                ...
                <List url='/zyb/propagateOfSupervise/list'  onError={this.handleError} onMount={this.onMount}>
                    <Filter cols={2}>
