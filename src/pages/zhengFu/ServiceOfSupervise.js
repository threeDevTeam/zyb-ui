...
//
import ServiceOfSuperviseDemoForm from '../ZhengfuDemoFrom/ServiceOfSuperviseDemoForm'
...
// @connect(({demo}) => ({demo}))
class ServiceOfSupervise extends PureComponent {
    state = { fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zyb/serviceOfSupervise/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            request('/zyb/serviceOfSupervise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        onOk: (values, hide) => {
                            request.post('/zyb/serviceOfSupervise/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/serviceOfSupervise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/serviceOfSupervise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
                ...
                <List url='/zyb/serviceOfSupervise/list'  onError={this.handleError} onMount={this.onMount}>
                    <Filter cols={2}>
...
export default ServiceOfSupervise