...
//
import EquipmentOfSuperviseDemoForm from '../ZhengfuDemoFrom/EquipmentOfSuperviseDemoForm'
...
// @connect(({demo}) => ({demo}))
class EquipmentOfSupervise extends PureComponent {
    state = { fileList: []
    }
    putFileToState = file => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    request.post('/zybadmin/equipmentOfSupervise/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            request('/zybadmin/equipmentOfSupervise/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/equipmentOfSupervise/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/equipmentOfSupervise/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
                ...
                <List url='/zybadmin/equipmentOfSupervise/list'  onError={this.handleError} onMount={this.onMount}>
                    <Filter cols={2}>
                    ...
                    <Table onRow={record => {
                        ...
                        <Table.Column title="购置时间" dataIndex="buyDate"/>
                        <Table.Column title="装备状态" dataIndex="status"/>
...
export default EquipmentOfSupervise