...
//
import ZhenduanTotalOfServiceDemoForm from '../JiShuDemoFrom/ZhenduanTotalOfServiceDemoForm'
...
class ZhenduanTotalOfService extends PureComponent {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <ZhenduanTotalOfServiceDemoForm option={{type}}/>,
                onOk: (values, hide) => {
                    request.post('/zybadmin/zhenduanTotalOfService/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/zhenduanTotalOfService/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            request('/zybadmin/zhenduanTotalOfService/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <ZhenduanTotalOfServiceDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            request.post('/zybadmin/zhenduanTotalOfService/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/zhenduanTotalOfService/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            ...
            <List url='/zybadmin/zhenduanTotalOfService/list'  onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
                ...
                <Table onRow={record => {
                    ...
                    <Table.Column title="诊断报告数" dataIndex="count3"/>
...
export default ZhenduanTotalOfService