...
class JianceTotalOfService extends PureComponent {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zybadmin/jianceTotalOfService/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            request('/zybadmin/jianceTotalOfService/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/jianceTotalOfService/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/jianceTotalOfService/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zybadmin/jianceTotalOfService/list'onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
