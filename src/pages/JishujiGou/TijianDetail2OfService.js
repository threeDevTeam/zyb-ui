...
//
import TijianDetail2OfServiceDemoForm from '../JiShuDemoFrom/TijianDetail2OfServiceDemoForm'
...
// @connect(({demo}) => ({demo}))
class TijianDetail2OfService extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zybadmin/tijianDetail2OfService/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/tijianDetail2OfService/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            request('/zybadmin/tijianDetail2OfService/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        onOk: (values, hide) => {
                            request.post('/zybadmin/tijianDetail2OfService/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/tijianDetail2OfService/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
        ...
        return (
            <List url='/zybadmin/tijianDetail2OfService/list' pageSize={2} onError={this.handleError} onMount={this.onMount}>
                ...
                <Table onRow={record => {
                    ...
                    <Table.Column title="疑似职业病大类名称" dataIndex="sickBigName"/>
                    <Table.Column title="疑似职业病小类名称" dataIndex="sickSmallName"/>
...
export default TijianDetail2OfService