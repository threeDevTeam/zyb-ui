...
class TijianDetail1OfService extends PureComponent {
        ...
        if ('create' === type) {
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/tijianDetail1OfService/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res && res.flag){
            ...
            request('/zyb/tijianDetail1OfService/getById?id=' + this.state.record.id).then(res => {
                if (res && res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/tijianDetail1OfService/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res && res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/tijianDetail1OfService/list' onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
