...
class TijianBasicOfService extends PureComponent {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request.post('/zyb/tijianBasicOfService/add', {data: {...values}}).then(res => {
                        if (res && res.flag) {
            ...
            request('/zyb/tijianBasicOfService/getById?id=' + this.state.record.id).then(res => {
                if (res && res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        onOk: (values, hide) => {
                            request.post('/zyb/tijianBasicOfService/edit', {data: {...values}}).then(res => {
                                if (res && res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zyb/tijianBasicOfService/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res && res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zyb/tijianBasicOfService/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res && res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zyb/tijianBasicOfService/list'onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
