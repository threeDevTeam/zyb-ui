...
//
import TijianBasicOfServiceDemoForm from '../JiShuDemoFrom/TijianBasicOfServiceDemoForm'
...
// @connect(({demo}) => ({demo}))
class TijianBasicOfService extends PureComponent {
    state = {fileList: []
    }
    putFileToState = file => {
    ...
    handleOperator = (type) => {
        ...
        if ('create' === type) {
            Dialog.show({
                ...
                // style: {width: 1000},
                enableValidate: true,
                content: <TijianBasicOfServiceDemoForm option={{type}}/>,
                onOk: (values, hide) => {
                    request.post('/zybadmin/tijianBasicOfService/add', {data: {...values}}).then(res => {
                        if (res.flag) {
            ...
            request('/zybadmin/tijianBasicOfService/getById?id=' + this.state.record.id).then(res => {
                if (res.flag) {
                    Dialog.show({
                        title: title,
                        footerAlign: 'label',
                        ...
                        // style: {width: '1000px'},
                        enableValidate: true,
                        content: <TijianBasicOfServiceDemoForm option={{type, record: res.data}}/>,
                        onOk: (values, hide) => {
                            request.post('/zybadmin/tijianBasicOfService/edit', {data: {...values}}).then(res => {
                                if (res.flag) {
                                    message.success("操作成功")
                                    hide()
                                    globalList.refresh()
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    ...
                    //异步请求
                    request.post('/zybadmin/tijianBasicOfService/exceladd',{method: 'post', data: formData}).then(res => {
                        if(res.flag){
            ...
            Dialog.show({
                ...
                onOk: (values, hide) => {
                    request('/zybadmin/tijianBasicOfService/delete?id=' + this.state.record.id).then(res => {
                        hide()
                        if (res.flag) {
                            globalList.refresh()
    ...
    render() {
        return (
            <List url='/zybadmin/tijianBasicOfService/list'onError={this.handleError} onMount={this.onMount}>
                <Filter cols={2}>
                    <Filter.Item label="机构名称" name="name"><Input/></Filter.Item>
                    <Filter.Item label="医院等级" name="hospitalLevel"><Input/></Filter.Item>
                </Filter>
...
export default TijianBasicOfService